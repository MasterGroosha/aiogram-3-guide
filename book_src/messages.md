...
**Учтите, что Telegram возвращает не сами значения, а их начало в тексте и длину.**
...
В отличие от `file_id`, идентификатора `file_unique_id` нельзя использовать для повторной отправки 
...
В случае в изображениями мы использовали не `message.photo`, а `message.photo[-1]`, почему? 
