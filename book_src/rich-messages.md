...
В следующем же обновлении, Bot API 10.2, к текстовой разметке добавили второй способ описывать такие сообщения — 
не строкой с тегами, а списком блоков-объектов. Глава рассчитана как раз на 10.2, поэтому оба подхода мы разберём. 
В этой главе поговорим про эти Messages, насколько они Rich.

![Изображение со скрытой ссылкой](images/rich-messages/zoey_vekselstein_meme.png){ loading=lazy }
...
Поддерживаются также `colspan`/`rowspan`. А рамки и «полосатый» стиль включаются атрибутами-флагами 
у самого `<table>` — `bordered` и `striped`, без значений; именно они и делают таблицу нарядной, 
...
* **Ошибку ловит pydantic, а не Telegram.** Забыли обязательное поле или перепутали тип — получите исключение 
  на этапе сборки объекта, а не `400 Bad Request` в ответ на запрос. IDE при этом подсказывает поля и их типы.
...
конструкторов незачем. Оба пути живые, выбирайте под задачу — ровно как и в случае с `sendMessage` 
...
| `InputRichBlockTable` | таблица `<table>` | `cells`, `is_bordered`, `is_striped`, `caption` |
...
в API нет — список становится нумерованным именно за счёт `value`/`type` у его элементов.
...
А `is_bordered` и `is_striped` — это ровно те же рамки и «зебра», что и атрибуты `bordered`/`striped` 
у HTML-тега `<table>` выше. Такая пара «поле блока ↔ атрибут тега» есть почти у каждой настройки, 
так что перенести уже написанный HTML на блоки обычно получается механически.
...
Начиная с Bot API 10.2 у тега `<tg-thinking>` есть блок-двойник, так что то же самое можно написать так:
    ...
    Во-вторых, Telegram рекомендует конкретный набор кастомных эмодзи специально для этого блока — 
    [@AIActions](https://telegram.dog/addemoji/AIActions). Вставить кастомный эмодзи в текст можно через 
