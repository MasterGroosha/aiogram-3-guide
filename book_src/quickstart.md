...
Такая возможность лучше всего подходит для передачи неизменяемых объектов, живущих в единственном экземпляре 
(чтобы не создавать отдельную [мидлварь](filters-and-middlewares.md#middlewares)). Впрочем, никто не мешает их 
редактировать. Чтобы получить значения в хэндлерах, просто укажите их как аргументы. Рассмотрим на примере:
